...
#include <fstream>
#include <streambuf>
#include "inverse-dynamics-tmp.hpp"
...
InverseDynamicsTMP::InverseDynamicsTMP(const std::string &name) :
	...
	this->addOperation("loadURDFAndSRDF", &InverseDynamicsTMP::loadURDFAndSRDF,
			this, RTT::ClientThread);

	this->addOperation("getKinematicChainNames",
			&InverseDynamicsTMP::getKinematicChainNames, this, RTT::ClientThread);

    this->addOperation("setDOFsize", &InverseDynamicsTMP::setDOFsize, this, RTT::ClientThread).doc("set DOF size");
    this->addOperation("setTranslationOnly", &InverseDynamicsTMP::setTranslationOnly, this, RTT::ClientThread).doc("set translation only, or use also orientation");
    this->addOperation("computeGravity", &InverseDynamicsTMP::computeGravity, this, RTT::ClientThread).doc("compute gravity compensating torques");
    this->addOperation("setBaseAndTip", &InverseDynamicsTMP::setBaseAndTip,this,RTT::ClientThread).doc("Set base and tip of the kinematic chain");
	this->addOperation("setGravityVector", &InverseDynamicsTMP::setGravityVector,this,RTT::ClientThread).doc("Set gravity vector direction and magnitude");
	...
	this->addOperation("useRobotInertia", &InverseDynamicsTMP::useRobotInertia_func,
...
void InverseDynamicsTMP::setTranslationOnly(const bool translationOnly) {
...
void InverseDynamicsTMP::setDOFsize(unsigned int DOFsize){
    assert(DOFsize > 0);
    this->DOFsize = DOFsize;
}

void InverseDynamicsTMP::useRobotInertia_func(bool useRobotInertia) {
	this->useRobotInertia = useRobotInertia;
}

void InverseDynamicsTMP::updateHook() {
...
bool InverseDynamicsTMP::startHook() {
	return _models_loaded;
}

bool InverseDynamicsTMP::configureHook() {
	return true;
}

std::vector<std::string> InverseDynamicsTMP::getKinematicChainNames() {
...
bool InverseDynamicsTMP::selectKinematicChain(const std::string& chainName) {
...
bool InverseDynamicsTMP::loadURDFAndSRDF(const std::string &URDF_path,
...
void InverseDynamicsTMP::calculateDynamics(
...
void InverseDynamicsTMP::computeGravity(rstrt::robot::JointState const & jointState, Eigen::VectorXf & gravity) {
    this->castEigenVectorFtoD(jointState.angles, jntPosConfigPlusJntVelConfig_q.q.data);
    this->castEigenVectorFtoD(jointState.velocities, jntPosConfigPlusJntVelConfig_q.qdot.data);
...
void InverseDynamicsTMP::castEigenVectorDtoF(Eigen::VectorXd const & d, Eigen::VectorXf & f) {
    f = d.cast <float> ();
}

void InverseDynamicsTMP::castEigenVectorFtoD(Eigen::VectorXf const & f, Eigen::VectorXd & d) {
    d = f.cast <double> ();
}

void InverseDynamicsTMP::castEigenMatrixDtoF(Eigen::MatrixXd const & d, Eigen::MatrixXf & f) {
    f = d.cast <float> ();
}

void InverseDynamicsTMP::castEigenMatrixFtoD(Eigen::MatrixXf const & f, Eigen::MatrixXd & d) {
    d = f.cast <double> ();
}

bool InverseDynamicsTMP::exists_test(const std::string& name) {
...
void InverseDynamicsTMP::setBaseAndTip(std::string base,std::string tip){
	this->base_string = base;
	this->tip_string = tip;
}
void InverseDynamicsTMP::setGravityVector(double x, double y, double z){
	gravity_vector = KDL::Vector(x,y,z);
}

ORO_LIST_COMPONENT_TYPE(cosima::InverseDynamicsTMP)
